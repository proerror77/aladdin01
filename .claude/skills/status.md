...
此时 Phase 5 进度仍按公式计算，但计数口径切换为“已完成变体数 / 总变体数”，并额外显示评分进度：
```text
ep01  [███████░░░] 72%  Phase 5: A/B 视频生成中 (16/22 变体完成)
                        A/B 评分：5/11 镜次已评分
```
...
ep01  [███████░░░] 74%  Phase 5: 视频生成中 (8/10 镜次完成)
...
交付目录：projects/{project}/outputs/ep01/deliverables/
```
...
Phase 5 的细分进度 = 50% + (已完成数量 / total_count) × 30%
- 常规模式：`已完成数量 = 已完成镜次数`，`total_count = total_shots`
- A/B 模式：`已完成数量 = 已完成变体数`，`total_count = 总变体数`
- 若 `total_count == 0`，默认返回 50%
- UI 展示百分比使用四舍五入到最近整数
...
ep01-ep20  [███████░░░] 74%  Phase 5: 视频生成中 (160/200 镜次完成)
