...
步骤：
1. 读取 `{ep}-shot-03-trace.jsonl`，获取失败原因和最终 prompt
2. 读取 `{ep}-phase2-trace.jsonl`，找到 shot-03 的 prompt 来源（assign_refs、assemble_prompts）
3. 读取 `{ep}-phase1-trace.jsonl`，检查是否有改写操作影响了该段内容
4. 对比原文 → 改写后 → prompt → API 拒绝，构建因果链
...
|---------|---------|
| 路径概览 | `session.jsonl` |
| 单集详情 | `{ep}-phase{1-4}-trace.jsonl` + `{ep}-shot-*-trace.jsonl` |
| 单 Phase | `{ep}-phase{N}-trace.jsonl` |
| 单 Shot | `{ep}-shot-{N}-trace.jsonl` |
| Backtrack | `{ep}-shot-{N}-trace.jsonl` + `{ep}-phase2-trace.jsonl` + `{ep}-phase1-trace.jsonl` |
