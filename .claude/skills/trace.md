...
```bash
ls -t projects/{project}/state/traces/ | head -1
```

**有 session-id**：直接使用 `projects/{project}/state/traces/{session-id}/`。
...
**索引加速**：定位 session 后，先检查 `projects/{project}/state/traces/{session-id}/index.json` 是否存在：
...
📊 Trace 详情：projects/{project}/state/traces/batch-20260329-143000/
```
...
**查看已有摘要**：读取 `projects/{project}/state/traces/{session-id}/summary.md` 并展示。

**重新生成**：
```bash
./scripts/api-caller.sh trace-summary projects/{project}/state/traces/{session-id}
```
...
当 session 结束时（或 `~trace` 首次查询该 session 时），自动生成 `projects/{project}/state/traces/{session-id}/index.json`：
