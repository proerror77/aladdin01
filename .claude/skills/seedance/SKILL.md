...
## 官方案例库精华（EvoLink awesome-seedance-2-guide）

来源：https://github.com/EvoLinkAI/awesome-seedance-2-guide
...
跟随镜头走出电梯，电梯外场景参考@图片3，男人环顾四周
```

### 多角色对白（直接写角色名+台词，无需复杂格式）
...
@图片3。镜头往前摇向红衣女特工，她走进一座豪宅消失不见了，豪宅参考@图片4。
全程不要切镜头，一镜到底。
```
...
| 问题 | 解决方案 |
|------|---------|
| 参考视频没复刻运镜 | 加 `完全参考@视频1的所有运镜效果` |
| 角色长相不一致 | 上传多角度图 + `保持角色外貌与@图片1完全一致` |
| 视频延长接缝不自然 | 延长 prompt 开头描述原视频最后一帧状态 |
| 多角色对话不准确 | 标清每个人名字、动作、情绪 |
| 方言/特定音色 | 直接写方言词汇 + `语气和音色参考@视频1` |
| 分镜不精准 | 上传分镜脚本图，比文字描述更精准 |
...
详细写法（标注语气，适合情绪复杂的镜次）：
```
[叶红衣]开口说道："出来吧，我的本命灵兽！"（语气：满怀期待）
[苏夜]画外音（内心独白）："这女的……召唤出来的是我？"（语气：震惊崩溃）
```
...
语气和音色参考@视频1
```
- 方言直接写进 prompt：四川话、粤语、豫剧腔等，模型能理解
- 多角色对话时标清每个人名字和动作，生成更准确

**分镜图输入**（比文字描述更精准）：
```
参考@图片1的分镜头脚本，参考@图片1的分镜、景别、运镜、画面，创作一段15s的[内容]
```
- 上传手绘或软件制作的分镜脚本图，模型直接读懂景别和镜头角度
- 适合有明确分镜设计的场景
