...
检测逻辑：
1. 对每个剧本，读取 `state/{ep}-phase{1-4}.json`
2. Phase 5 进度：统计 `state/{ep}-shot-*.json` 中 `status: completed` 的数量
3. 确定每个剧本的起始阶段
...
所有参考图已由 `~design` 预先生成并锁定到 `state/design-lock.json`。Phase 3 只做校验：检查每个镜次引用的参考图是否存在。

```
[ep01] spawn design-agent（校验参考图完整性）
[ep02] spawn design-agent（校验参考图完整性）
[ep03] spawn design-agent（校验参考图完整性）
