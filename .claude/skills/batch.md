...
在 Phase 3 并行执行前，构建全局角色注册表：

```
1. 检查 assets/characters/profiles/ 是否已有角色档案（preprocess 产出）
2. 如果有：直接从 profiles 读取角色列表、tier、first_episode
3. 如果没有：遍历所有 outputs/{ep}/render-script.md，提取角色列表
4. 去重，生成全局角色列表
5. 检查 assets/characters/images/ 中已有角色
6. 输出角色注册表到 state/character-registry.json
```
...
{
  "characters": {
    "凌霄": {"first_ep": "jiuba-ep01", "tier": "protagonist", "status": "pending"},
    "徐莺莺": {"first_ep": "jiuba-ep01", "tier": "protagonist", "status": "pending"},
    "王象": {"first_ep": "jiuba-ep04", "tier": "minor", "status": "pending"}
  }
}
```
