...
在 Phase 2 完成后，为每集生成本地 fallback 分镜图或 AI 分镜图，并把 `storyboard_image_path` 注入 `visual-direction.yaml`：
...
for ep in episodes:
  ...
  # 写入状态文件
  echo '{"episode":"'${ep}'","phase":3.5,"status":"completed","started_at":"...","completed_at":"...","data":{"shot_packets_generated":'$(ls projects/{project}/state/shot-packets/${ep}-shot-*.json | wc -l)'}}' > projects/{project}/state/${ep}-phase3.5.json
done
```
