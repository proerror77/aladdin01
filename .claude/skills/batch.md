...
for ep in episodes:
  ...
  # 写入状态文件
  echo '{"episode":"'${ep}'","phase":3.5,"status":"completed","started_at":"...","completed_at":"...","data":{"shot_packets_generated":'$(ls projects/{project}/state/shot-packets/${ep}-shot-*.json | wc -l)'}}' > state/${ep}-phase3.5.json
done
```
