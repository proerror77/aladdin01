...
2. **多视图角色**：front 图用 `@图片N 作为<角色名>`，side/back 图在开头声明用途：
   ```
   @图片1 作为<苏夜（青玉蚕形态）>，@图片2 和 @图片3 为苏夜侧面/背面参考（保持形态一致性）
   ```
3. **时间戳格式**：`0-3秒：` `4-8秒：` `9-12秒：` `13-15秒：`（覆盖全部时长，不留空白）
4. **台词格式**：`[角色名]（情绪）："台词内容"` 或 `画外音（角色名内心独白）："台词"（语气：情绪）`
5. **音效单独一行**：在时间戳分段之后，禁止项之前
6. **禁止项**：每个 prompt 结尾必须有 `禁止出现字幕、对话框、水印、Logo`
7. **分镜图引用**：由 storyboard-agent（Phase 2.3）在生成分镜图后自动注入 `@图片N`，visual-agent 不需要手动添加
8. **不写进 prompt 的内容**：比例（9:16）、分辨率（720p）——这些是 API 参数
