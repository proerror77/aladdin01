...
- **时间戳分镜法**：按镜次实际时长动态拆分时间戳。默认 shot 为 13-15 秒，收尾 shot 可为 10-12 秒；每段通常 2-3 秒，至少 3 段，并覆盖全部时长
...
**每个 shot 的时长规范**：
- 标准 shot：默认 13-15 秒，内含 4-5 个时间戳分段（每段通常 2-3 秒）
- 收尾 shot：10-12 秒，内含 3-4 个时间戳分段（每段通常 2-3 秒）
...
Step 5: 为每个 shot 分配时长（默认 13-15 秒，收尾可 10-12 秒），按实际时长安排时间戳分段
```
...
- shot_id: "ep01-shot-01"
  shot_index: 1
  duration: 15  # 秒，默认 13-15 秒/shot；收尾 shot 可 10-12 秒，内含至少 3 个时间戳分段
...
⚠️ **核心规则**：每个 shot 都是一个完整叙事单元。默认 shot 为 13-15 秒，收尾 shot 可为 10-12 秒；prompt 内按**实际时长**用时间戳分镜法控制镜头切换。每段通常 2-3 秒，至少 3 段，并覆盖全部时长。参考 `.claude/skills/seedance/SKILL.md` 的「时间戳分镜法」章节。
...
构图参考@图片N               ← 构图/景别参考（storyboard-agent 填写）
```

**标准格式（TikTok 短剧，默认 13-15 秒，有参考图）**：

> ⚠️ **一致性核心原则（来自即梦实战验证）**：
> - **方法一：人物与场景分段描述**——人物外貌/动作 和 场景环境 必须分开写，不能混在同一句话里。混写会让模型在"风格与结构"上产生弱约束，加大一致性难度。
> - **方法二：每个时间戳只描述一个镜头**——一个时间戳分段只对应一个镜头动作，不能在同一分段里堆叠多个镜头切换（如"推近→光芒散去→法阵亮起"是三个镜头，必须拆成三个时间戳）。镜头不拆开，画面永远只是漂浮的、不连贯的。
...
[人物描述段 — 从角色档案注入，必须单独成段，不能与场景混写]
{角色名}外貌：{从角色档案 appearance/variant.appearance 字段直接复制}
（多角色时逐一列出，每个角色独立一行）

[场景描述段 — 必须单独成段，不能与人物描述混写]
场景：{环境、光线、色调、空间感}

[参考图声明]
@图片1 作为<角色1>，@图片2 作为<角色2>，场景参考@图片3

[时间戳分镜 — 按实际时长动态拆分，每个时间戳只对应一个镜头，不堆叠多个动作]
默认 13-15 秒示例：
0-3秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）："台词内容"（语气：情绪）}
4-6秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}
7-9秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}
10-12秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}
13-15秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}

收尾 10-12 秒示例：
0-3秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}
4-6秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}
7-9秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}
10-12秒：{景别+运镜}，{角色}做{单一动作}，{台词（如有）}
...
1. **人物与场景必须分段描述（强制）**：人物外貌/动作描述 和 场景环境描述 必须分开成独立段落，不能混写在同一句话里。混写会产生"风格与结构弱约束"，加大一致性难度。
   ```text
   # 错误（混写）：
   0-3秒：叶红衣站在月光下的树林中，红衣飘动，捏碎召唤石
   
   # 正确（分段）：
   叶红衣外貌：18岁红衣少女，红衣似火但破损，高冷御姐脸，眼神倔强落寞
   场景：月光透过树梢，地面法阵金光隐现，空气中漂浮着灵气粒子
   0-3秒：近景固定，[叶红衣]颤抖着捏碎召唤石，鲜血滴落
   ```
2. **每个时间戳只对应一个镜头（强制）**：一个时间戳分段只描述一个镜头动作，不能堆叠多个镜头切换。镜头不拆开，画面永远只是漂浮的、不连贯的。
   ```text
   # 错误（一个时间戳堆叠多个镜头）：
   5-8秒：镜头缓慢推近，一道绿光从树梢坠落，光芒散去，法阵中央趴着拇指大小的青玉蚕
   
   # 正确（每个镜头独立时间戳）：
   5-6秒：全景固定，一道绿光从树梢坠落
   7-8秒：近景推镜头，光芒散去，法阵中央趴着拇指大小的青玉蚕
   ```
3. **有参考图时**：开头用 `@图片N 作为<角色名>` 声明，时间戳描述中加 `保持角色外观与@图片N一致`
4. **有运镜参考视频时**：在 `@图片N` 声明之后追加 `参考@视频1的运镜效果`（或更具体地描述，如 `参考@视频1的追逐跟拍节奏`）。`@视频1` 固定为第一个视频，索引从 1 开始独立计数（与图片编号互不干扰）
5. **多视图角色**：front 图用 `@图片N 作为<角色名>`，side/back 图在开头声明用途：
   ```text
   @图片1 作为<苏夜（青玉蚕形态）>，@图片2 和 @图片3 为苏夜侧面/背面参考（保持形态一致性）
   ```
6. **时间戳格式**：按实际 shot 时长动态编排，分段通常为 2-3 秒、至少 3 段，并覆盖全部时长，不留空白。
   - 默认 13-15 秒示例：`0-3秒：` `4-6秒：` `7-9秒：` `10-12秒：` `13-15秒：`
   - 收尾 10-12 秒示例：`0-3秒：` `4-6秒：` `7-9秒：` `10-12秒：`
7. **台词格式**：`[角色名]（情绪/语气/语速）："台词内容"` — 必须同时标注三项：
   - **情绪**：惊恐/崩溃/决绝/得意/懵逼/愤怒/嘲讽...
   - **语气**：轻声/怒吼/颤抖/平静/嘶吼/低语...
   - **语速**：急促/缓慢/正常/拖长/断断续续...
   - 示例：`[叶红衣]（崩溃/颤抖/断断续续）："青……青玉蚕？是最废物的凡胎级昆虫？"`
   - 画外音格式：`画外音（苏夜内心独白/惊恐/急促）："这特么是哪？！"`
8. **音效单独一行**：在时间戳分段之后，禁止项之前
9. **禁止项**：每个 prompt 结尾必须有 `禁止出现字幕、对话框、水印、Logo`
10. **分镜图引用**：由 storyboard-agent（Phase 2.3）在生成分镜图后自动注入 `@图片N`，visual-agent 不需要手动添加
11. **不写进 prompt 的内容**：比例（9:16）、分辨率（720p）——这些是 API 参数

**⚠️ 禁止的写法**：
```text
# 错误1：把每个镜头切换拆成独立 shot（太细，浪费请求次数）
shot-01 (4s): 复眼视角
shot-02 (4s): 拉远发现是蚕
shot-03 (4s): 滚圈
shot-04 (4s): 绝望 OS
...
shot-01 (15s): 0-3秒复眼视角，4-6秒拉远，7-9秒滚圈，10-12秒绝望OS

# 错误2：人物与场景混写（弱约束，一致性差）
0-3秒：叶红衣站在月光下的树林中，红衣飘动，捏碎召唤石

# 正确：人物描述和场景描述分段
叶红衣外貌：18岁红衣少女，红衣似火但破损，高冷御姐脸
场景：月光透过树梢，地面法阵金光隐现
0-3秒：近景固定，[叶红衣]颤抖着捏碎召唤石，鲜血滴落

# 错误3：一个时间戳堆叠多个镜头（画面漂浮，不连贯）
5-8秒：镜头缓慢推近，一道绿光从树梢坠落，光芒散去，法阵中央趴着拇指大小的青玉蚕
...
- [ ] **人物描述与场景描述已分段**：角色外貌/动作描述 和 场景环境描述 各自独立成段，没有混写在同一句话里
- [ ] **每个时间戳只对应一个镜头**：没有在同一时间戳分段里堆叠多个镜头切换（如"推近→光芒散去→法阵亮起"必须拆成三个时间戳）
