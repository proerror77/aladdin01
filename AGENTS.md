...
**状态机入口**：`scripts/pipeline-runner.py`
**Phase 定义**：`config/pipeline/phases.yaml`

---

## 状态机（pipeline-runner.py）

所有 Phase 流转由 `scripts/pipeline-runner.py` 管理，不依赖 LLM 解释伪代码。
...
python3 scripts/pipeline-runner.py shot-status --project qyccan --ep ep01 --shot ep01-shot-03
python3 scripts/pipeline-runner.py shot-event --project qyccan --ep ep01 --shot ep01-shot-03 --event gen_success
```
...
config/pipeline/phases.yaml               Phase 定义
scripts/pipeline-runner.py                状态机
scripts/api-caller.sh                     统一 API 调用
scripts/signal-watcher.py                 信号目录监控（调试）
```
...
| 文件 | 说明 |
|------|------|
| `projects/{project}/outputs/{ep}/videos/shot-{N}.mp4` | 生成视频 |
...
| 生成视频 | `projects/{project}/outputs/{ep}/videos/{shot_id}.mp4` |
| 世界模型 | `projects/{project}/state/ontology/{ep}-world-model.json` |
...
**CLI**:
```bash
python3 scripts/pipeline-runner.py shot-event \
  --project qyccan --ep ep01 --shot ep01-shot-03 --event gen_success
```
