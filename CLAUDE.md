...
# 单剧本模式：将剧本放入 projects/{name}/script/ 后运行
~start

# 批量模式：将多个剧本放入 projects/{name}/script/ 后运行
~batch                      # 从头开始
~batch --resume             # 断点续传
...
输出：`projects/{name}/state/ontology/{ep}-world-model.json`
...
输出：`projects/{name}/assets/packs/characters/`, `projects/{name}/assets/packs/scenes/`, `projects/{name}/assets/packs/props/`
...
输出：`projects/{name}/state/shot-packets/{ep}-shot-{N}.json`
...
每个专案独立存放在 `projects/{project-name}/` 下，包含自己的剧本、资产、状态和产出。

```
projects/
├── {project-name}/          # 专案根目录（如 jiuba、qyccan）
│   ├── script/              # 分集剧本（ep01.md, ep02.md ...）
│   ├── assets/              # 专案专属资产
│   │   ├── characters/
│   │   │   ├── images/      # v1.0: 参考图（单张）
│   │   │   ├── profiles/    # 角色档案（.yaml）
...
│   │       └── props/       # 道具包（状态变体）
│   ├── state/               # 专案状态（独立状态文件，避免并发冲突）
│   │   ├── shot-packets/    # v2.0: Shot Packet（{ep}-shot-{N}.json）
...
│           └── videos/      # 分镜视频（shot-{N}.mp4）
raw/             # 原始剧本文件（.docx/.md/.txt，~preprocess 处理后输出到 projects/{name}/script/）
